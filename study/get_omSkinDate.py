...
class Get_omSkinDate:
    def __init__(self , skinNode , skinGeo):
        ...
        influece_objs = [inf.fullPathName() for inf in infs]
        ...
        self.influece_objs = influece_objs
        self.weights = weights 

def setSkin(SkinData , SkinNode):
    selects_list = om.MSelectionList()
    selects_list.add(SkinNode)

    skinObj = selects_list.getDependNode(0)
    Mfn_skin = oma.MFnSkinCluster( skinObj)

    Mgeo = Mfn_skin.getOutputGeometry()
    Mgeo = om.MDagPath.getAPathTo(Mgeo[0])

    select_compos = om.MGlobal.getSelectionListByName("{}.vtx[*]".format(Mgeo.fullPathName()))
    compos = select_compos.getComponent(0)[1]

    inf_indices = om.MIntArray()
    for i in range(len(SkinData.influece_objs)):
        inf_indices.append(i)
    
    weights = om.MDoubleArray()
    for weight in SkinData.weights:
        weights.append(weight)

    Mfn_skin.setWeights(Mgeo , compos , inf_indices , weights , False)
