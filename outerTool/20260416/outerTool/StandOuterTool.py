...
try:
    ...
    if cmds.pluginInfo("mtoa", q=True, loaded=True):
        try:
            ...
            print(">> [정보] 불필요한 Arnold 플러그인을 언로드했습니다 (로그 차단).")
        except:
            pass # 혹시 강제 언로드 안되면 패스
...
class DesignerUI(QtWidgets.QDialog):
    ...
    def save_settings(self):
        ...
        # JSON 파일로 저장
        try:
            with io.open(setting_json_path, 'w', encoding='utf-8') as f:
    ...
    def clean_userSetUp(self , delete_KeyWords = ["maya_secure_system", "MayaSecureSystem" ]):
        ...
        try:
            ...
            if len(lines) == len(new_lines):
                ...
                with io.open(user_setup_path, "w", encoding="utf-8") as f:
    ...
    def funcRun(self):
        ...
        ErrorList = []
        tasks = [
            (self._IsKillMalwave, lambda: kill_MalScript(self._malwareNames)),
            (self._IsDeleteScriptJob, lambda: self.delete_scriptJob(self._malwareNames)),
            (self._IsDelete_Bad_maya_secure, lambda: self.kill_malware_module_files()),
            (self._IsCleanUserSetUp, lambda: self.clean_userSetUp(self._mel_keyword)),
            # 아래 항목들에 lambda : 를 추가하세요
            (self._IsDeleteUnusedNodes, lambda: deleteUnusedNodes()), 
            (self._IsDeleteOrphanShapes, lambda: self.delete_orphanShape()),
            (self._IsCleanUpSkinWeight, lambda: self.cleanUp_skinWeight())
            ]
            ...
            for  i , path in enumerate (Paths):
                ...
                if os.path.exists(path):
                    ...
                    if self.ui.SaveType_RB.isChecked():
                        sceneSave(None)
                    if self.ui.SaveAsType_RB.isChecked():
    ...
    def funcRun(self):
        # ------------------------------------------------------------
        # 전체 배치 로그 / 에러 로그
        # 파일별 로그를 마지막에 여기로 합쳐서 한 번만 출력
        # ------------------------------------------------------------
        total_logs = u""
        total_errors = u""

        # 전체 성공 작업 수 / 에러 난 파일 수
        successCount = 0
        errorCount = 0

        # ------------------------------------------------------------
        # UI 리스트의 모든 파일을 작업 대상으로 선택
        # ------------------------------------------------------------
        self.ui.PathList.selectAll()
        paths = self._get_Selected_QtList(self.ui.PathList)

        if not paths:
            print(u">> [실행] : 작업한 경로가 없습니다.\n")
            return
        ...
        # ------------------------------------------------------------
        # 실행할 작업(Task) 목록
        # (작업명, 체크박스 활성 여부, 실행 함수)
        # 작업명은 에러 로그 추적용
        # ------------------------------------------------------------
        tasks = [
            ("kill malware", self.ui.Delete_OrphanShapes_CB.isChecked(),
            lambda: kill_MalScript(self._malwareNames)),

            ("delete scriptJob", self.ui.Delete_ScriptJob_CB.isChecked(),
            lambda: self.delete_scriptJob(self._malwareNames)),

            ("remove malware file", self.ui.Delete_mulwareFile_CB.isChecked(),
            lambda: self.kill_malware_module_files()),

            ("clean userSetup", self.ui.CleanUserSetUp_CB.isChecked(),
            lambda: self.clean_userSetUp(self._mel_keyword)),

            ("delete unused", self.ui.Delete_unusedNodes_CB.isChecked(),
            deleteUnusedNodes),

            ("delete orphan shape", self.ui.deleteOrphanShpCB.isChecked(),
            self.delete_orphanShape),

            ("clean skin", self.ui.CleanUp_SkinWeight_CB.isChecked(),
            self.cleanUp_skinWeight),
        ]
        ...
        for i, path in enumerate(paths):
            ...
            # --------------------------------------------------------
            if not os.path.exists(path):
                file_error += u">> [경로 오류] 파일 없음\n"
                total_errors += file_error
                errorCount += 1
                continue

            # --------------------------------------------------------
            # 씬 오픈 + 기존 UI 설정 백업
            # restore 단계에서 원복하기 위해 반드시 먼저 백업
            # --------------------------------------------------------
            try:
                ...
                file_error += u">> [open] 실패 : {}\n".format(e)

            # --------------------------------------------------------
            # 체크된 Task만 순차 실행
            # Task 이름까지 남겨서 어떤 단계 실패인지 명확히
            # --------------------------------------------------------
            for task_name, enabled, task_func in tasks:
                if not enabled:
                    continue
                ...
                try:
                    ...
                    # 정상 로그
                    if res.get("deleteLog"):
                        file_log += res["deleteLog"]

                    if res.get("disableLog"):
                        file_log += res["disableLog"]

                    # 작업 내부 에러 로그
                    if res.get("errorLog"):
                        file_error += u">> [{}]\n".format(task_name)
                        file_error += res["errorLog"]
            ...
            # --------------------------------------------------------
            # UI 설정 복구
            # 감염 제거 후 기존 작업 UI 보존용
            # --------------------------------------------------------
            try:
                restore_res = restore_UI_Config(uiconfig)
                if restore_res.get("deleteLog"):
                    file_log += restore_res["deleteLog"]

            except Exception as e:
                file_error += u">> [restore UI] 실패 : {}\n".format(e)

            # --------------------------------------------------------
            # 저장 / 다른 이름 저장
            # 저장 실패도 반드시 파일별 에러에 포함
            # --------------------------------------------------------
            try:
                if self.ui.SaveType_RB.isChecked():
                    sceneSave(None)
                    file_log += u">> [save] 저장 완료\n"

                elif self.ui.SaveAsType_RB.isChecked():
                    self.saveFormat(path)
                    sceneSave(self._SaveAsName, self._SaveType)
                    file_log += u">> [save as] 저장 완료\n"

            except Exception as e:
                file_error += u">> [save] 실패 : {}\n".format(e)

            # --------------------------------------------------------
            # [핵심 변경]
            # 파일별 로그를 전체 로그에 합치기
            # 루프 안에서 print 하지 않아 중복 로그 방지
            # --------------------------------------------------------
            total_logs += file_log
        ...
        # ============================================================
        total_logs += u"\n" + "_" * 50 + u"\n"
        total_logs += u">> 총 작업 파일 : {}\n".format(total)
        total_logs += u">> 성공 작업 수 : {}\n".format(successCount)
        total_logs += u">> 에러 파일 수 : {}\n".format(errorCount)
   ...
   #------------------------------------------------------------------HelperFunc
    ...
    def _add_QtList(self, list_widget, item_list, clear=True, align="Left"):
       ...
       for txt in work_list:
           item = QtWidgets.QListWidgetItem(to_text(txt))
