...
def sceneSave(SaveAsPath = None , FileType = 'mayaBinary'):
    try:
        ...
        cmds.dgeval(cmds.ls(type="transform")) # 혹은 필요한 주요 노드 강제 연산
    except:
        pass
...
def clean_skining(targetSkinCluster, dicts, threshold=0.001, raiseBool=False):
    """
    스킨 웨이트 값을 지정한 임계치 기준으로 정리하기
    Args:
        -targetSkinCluster (str): 타겟 스킨 클러스터 노드
        -dicts             (dict): 스킨 웨이트 정보 딕셔너리
        -threshold         (float): 무시할 웨이트 값의 임계치
        -raiseBool         (bool): 에러 발생 시 강제 예외 처리 여부
    Returns:
        작업 완료 여부(bool), 작업 로그(str)
    """
    string = u""
    IsDone = False
    ...
    if not cmds.objectType(targetSkinCluster) == "skinCluster":
        if raiseBool:
            raise ValueError(u">> Invalid Input. Expected a 'skinCluster' type.")
        else:
            return IsDone, string 
            
    if not isinstance(dicts, dict):
        if raiseBool:
            raise ValueError(u">> Invalid Input. Expected a 'dicts' type.")
        else:
            return IsDone, string 
            ...
            for jnt, w in w_info.items():
                if w == 0.0:
                    ...
                    if w <= threshold:
                        ...
                        vtxCount += 1
                        string  += u"processing... workingTotal :({}/{}), currentWork : {}={}\n".format(vtxCount, Total, vtx, jnt)
